...
def system_prompt(branch: str, repo: Path) -> str:
    return (
        ...
        "- delegate(): hand large reads, searches, or long log dumps to a "
        "sub-agent and keep its digest.\n"
        "- run_review(): ask the reviewer sub-agent about the diff before "
        "finishing when the change is non-trivial.\n"
        "- escalate(): stop and ask a human when blocked by ambiguity or a "
        "decision you must not make alone. In unattended runs your escalation "
